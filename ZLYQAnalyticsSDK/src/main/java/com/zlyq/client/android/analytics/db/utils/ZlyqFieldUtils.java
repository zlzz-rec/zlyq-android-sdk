...
import com.zlyq.client.android.analytics.db.annotations.ZlyqManyToOne;
import com.zlyq.client.android.analytics.db.annotations.ZlyqOneToMany;
import com.zlyq.client.android.analytics.db.annotations.ZlyqProperty;
...
public class ZlyqFieldUtils {
	...
	public static Field getFieldByColumnName(Class<?> clazz, String columnName){
		...
		if(columnName!=null){
			...
			if(fields!=null && fields.length>0){
				if(columnName.equals(ZlyqClassUtils.getPrimaryKeyColumn(clazz)))
					field = ZlyqClassUtils.getPrimaryKeyField(clazz);
					
				if(field == null){
					for(Field f : fields){
						ZlyqProperty zlyqProperty = f.getAnnotation(ZlyqProperty.class);
						if(zlyqProperty !=null && columnName.equals(zlyqProperty.column())){
							field = f;
							break;
						}
						
						ZlyqManyToOne zlyqManyToOne = f.getAnnotation(ZlyqManyToOne.class);
						if(zlyqManyToOne !=null && zlyqManyToOne.column().trim().length()!=0){
							field = f;
							break;
						}
					}
	...
	public static String getColumnByField(Field field){
		ZlyqProperty zlyqProperty = field.getAnnotation(ZlyqProperty.class);
		if(zlyqProperty != null && zlyqProperty.column().trim().length() != 0){
			return zlyqProperty.column();
		}
		
		ZlyqManyToOne zlyqManyToOne = field.getAnnotation(ZlyqManyToOne.class);
		if(zlyqManyToOne !=null && zlyqManyToOne.column().trim().length()!=0){
			return zlyqManyToOne.column();
		}
		
		ZlyqOneToMany zlyqOneToMany = field.getAnnotation(ZlyqOneToMany.class);
		if(zlyqOneToMany !=null && zlyqOneToMany.manyColumn()!=null && zlyqOneToMany.manyColumn().trim().length()!=0){
			return zlyqOneToMany.manyColumn();
		}
	...
	public static String getPropertyDefaultValue(Field field){
		ZlyqProperty zlyqProperty = field.getAnnotation(ZlyqProperty.class);
		if(zlyqProperty != null && zlyqProperty.defaultValue().trim().length() != 0){
			return zlyqProperty.defaultValue();
		}
		return null ;
	}
	...
	public static boolean isTransient(Field f) {
		return f.getAnnotation(ZlyqTransient.class) != null;
	}
	...
	public static boolean isManyToOne(Field field){
		return field.getAnnotation(ZlyqManyToOne.class)!=null;
	}
	
	public static boolean isOneToMany(Field field){
		return field.getAnnotation(ZlyqOneToMany.class)!=null;
	}
